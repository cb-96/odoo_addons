...
class FederationDynamicPairing(models.AbstractModel):
    ...
    def swiss_pairs(self, participants, previous_pairs=None, standings=None):
        ...
        points = standings or {}
        ordered = sorted(team_ids, key=lambda team_id: (-points.get(team_id, 0), team_id))
        bye = ordered.pop() if len(ordered) % 2 else False
        pairs = []
        while ordered:
            home = ordered.pop(0)
            opponent_index = next(
                (index for index, away in enumerate(ordered) if frozenset((home, away)) not in previous),
                None,
            )
            if opponent_index is None:
                raise ValidationError(_("No repeat-free Swiss pairing is available for the current score group."))
    ...
    def ladder_challenge(self, challenger_rank, opponent_rank, max_distance=3):
        if challenger_rank <= opponent_rank:
            raise ValidationError(_("A ladder challenge must target a higher-ranked opponent."))
        if challenger_rank - opponent_rank > max_distance:
            raise ValidationError(_("The opponent is outside the allowed challenge distance."))
        return {"allowed": True, "winner_rank": opponent_rank, "loser_rank": challenger_rank}
    ...
    def double_elimination_routes(self, participant_count):
        ...
        if participant_count < 4 or participant_count & (participant_count - 1):
            raise ValidationError(_("Double elimination requires a power-of-two field of at least four teams."))
