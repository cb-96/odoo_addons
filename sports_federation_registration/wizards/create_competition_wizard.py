...
class FederationCreateCompetitionWizard(models.TransientModel):
    ...
    setup_template = fields.Selection(
        [
            ("league", "League"),
        ...
        default="league",
        required=True,
        string="Setup Template",
        help="Applies reversible defaults; every value remains editable before creation.",
    )
    ...
    def _onchange_setup_template(self):
        for line in self.division_line_ids:
            line.tournament_type = (
                "single_day" if self.setup_template == "tournament_day" else "multi_day"
            )
