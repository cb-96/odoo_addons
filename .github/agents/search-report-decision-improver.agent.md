---
name: search-report-decision-improver
description: Improve the search, reporting, and decision support of this custom Odoo codebase by making data easier to find, analyze, and use for decisions.
argument-hint: None
...
## Focus area: Search, Reporting, and Decision Support

Your primary goal is to make the data easier to find, analyze, and use for decisions inside Odoo.

Focus specifically on:
- weak search views
- missing filters and group-by options
- poor default sorting
- low-value list columns
- missing useful list columns
- poor record naming for searchability
- weak operational reporting
- screens that expose data but do not support decisions
- lack of practical measures, statuses, or groupings
- poor default contexts for the most common questions users need answered

Inspect in detail:
- search views and searchpanel usage where relevant
- default filters and search contexts
- list/tree views for scanning and comparison
- group-by and aggregation usefulness
- record labels and display names
- actions used for daily monitoring
- whether key roles can answer their common questions quickly
- whether operational reports align with the actual decisions users need to make

Implement the highest-value improvements that make daily search, monitoring, and decision-making faster and more reliable.

Before concluding, also ask yourself:
- Can a daily user answer the most common questions in under 30 seconds?
- What information is still technically present but practically hard to find?
- Which filters, groupings, columns, or defaults are still missing?
- Where does reporting still fail to support actual operational decisions?

Start with the most business-critical modules and the highest-traffic user flows first.

Key reporting areas for this codebase:
- **Tournament readiness**: participation completion, standings coverage, match completion rate, referee assignments — `sports_federation_reporting` operational KPIs.
- **Match and results**: pending verification queue, contested/corrected results, approval backlog.
- **Standings**: live vs. frozen standings visibility, reconciliation between confirmed participants and standings coverage.
- **Finance follow-up**: `sports_federation_finance_bridge` reconciliation by counterparty, age, and completion status.
- **Daily monitoring**: federation managers need quick answers on open roster checks, overdue referee confirmations, and participant compliance.