...
class FederationCompetitionIntegrityService(models.AbstractModel):
    ...
    def scan_division(self, division_id):
        ...
        if foreign_matches:
            add("foreign_match_stage", _("Matches reference a stage outside this division."), foreign_matches)

        foreign_gamedays = division.round_ids.filtered(
            lambda day: day.stage_id and day.stage_id.id not in stage_ids
        )
        if foreign_gamedays:
            add("foreign_gameday_stage", _("Gamedays reference a stage outside this division."), foreign_gamedays)
        ...
        if foreign_progressions:
            add("foreign_progression_stage", _("Progression rules leave the division stage graph."), foreign_progressions)

        self_cycles = progressions.filtered(
            lambda rule: rule.source_stage_id == rule.target_stage_id
        )
        if self_cycles:
            add("progression_self_cycle", _("A progression rule targets its own source stage."), self_cycles)

        if division.workspace_stage_id and division.workspace_stage_id.id not in stage_ids:
            add("invalid_workspace_stage", _("The default workspace stage is outside this division."))
        if division.workspace_knockout_stage_id and division.workspace_knockout_stage_id.id not in stage_ids:
            add("invalid_knockout_stage", _("The default knockout stage is outside this division."))
    ...
    def assert_division_integrity(self, division_id):
        ...
        if not result["valid"]:
            messages = "\n".join("- %s" % issue["message"] for issue in result["issues"])
            raise ValidationError(_("Competition integrity checks failed:\n%(issues)s", issues=messages))
        return result
