...
public_sources = "\n".join(
    ...
    for base in (
        ROOT / "sports_federation_public_site",
        ROOT / "sports_federation_portal",
    )
...
checks = {
    ...
    "no tournament route namespace": all(
        token not in public_sources
        for token in (
            "/" + "tournaments",
            "/" + "tournament/",
            "/api/v1/" + "tournaments",
    ...
    "competition registration route": "/competitions/<string:edition_slug>/register"
    in legacy_controller,
    "competition division calendar route": "/competitions/<string:edition_slug>/divisions/<string:division_slug>/schedule.ics"
    in legacy_controller,
