...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Performance qualification contract is unreadable: {error}",
            file=sys.stderr,
        )
        return 1
    ...
    for relative, class_names in required_classes.items():
        ...
        for class_name in class_names:
            if class_name not in available:
                errors.append(
                    f"missing performance test class {class_name} in {relative}"
                )
    if errors:
        ...
        return 1
    print(
        f"Performance qualification contract passed: {len(budgets)} budgets, {len(required_files)} test files."
    )
    return 0
