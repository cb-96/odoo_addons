...
def test_contract_is_valid():
    contract = json.loads(Path(__file__).with_name("release_invariants.json").read_text())
    capture.validate_contract(contract)


def test_comparison_accepts_preserved_counts_and_clean_integrity():
    before = {"counts": {"clubs": 3}, "count_modes": {"clubs": "exact"}, "zero_checks": {"orphans": 0}}
...
def test_minimum_count_mode_allows_growth_but_not_loss():
    before = {"counts": {"attachments": 3}, "count_modes": {"attachments": "minimum"}, "zero_checks": {}}
    assert compare.compare(before, {"counts": {"attachments": 4}, "zero_checks": {}}, "after") == []
    assert compare.compare(before, {"counts": {"attachments": 2}, "zero_checks": {}}, "after")
    assert compare.compare(before, {"counts": {"attachments": 4}, "zero_checks": {}}, "rollback", force_exact=True)
