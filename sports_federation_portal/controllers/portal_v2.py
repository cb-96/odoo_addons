...
class FederationPortalV2(FederationPortalBase):
    ...
    def portal_my_competition_entries(self, **kw):
        teams = request.env["federation.portal.v2.scope"].teams(user=request.env.user)
    ...
    def portal_create_competition_entry(self, window_id, team_id, **kw):
        try:
            ...
            request.env["federation.portal.v2.scope"].assert_team(
    ...
    def legacy_workspace_redirect(self, page=1, **kw):
        return redirect("/my/competitions", code=302)
    ...
    def legacy_workspace_detail_redirect(self, tournament_id, team_id, **kw):
        tournament = request.env["federation.tournament"].browse(tournament_id).exists()
        if not tournament or not tournament.edition_id:
            return redirect("/my/competitions", code=302)
        return redirect(f"/my/competitions/{tournament.edition_id.id}", code=302)
    ...
    def legacy_tournament_operations_redirect(self, tournament_id, **kw):
        tournament = request.env["federation.tournament"].browse(tournament_id).exists()
        day = request.env["federation.matchday"].search(
            [
                ("edition_id", "=", tournament.edition_id.id if tournament else 0),
        ...
        return redirect(
            f"/sports/match-days/{day.id}/operations" if day else "/my/match-days",
            code=302,
        )
