...
class TestProductionSecurityMatrix(TransactionCase):
    ...
    def test_mutating_portal_routes_declare_post_and_csrf(self):
        ...
        self.assertFalse(violations, "POST routes missing explicit CSRF: %s" % violations)

    def test_qol_bulk_routes_require_manager_group(self):
        source = (self.portal_root / "controllers" / "qol.py").read_text(encoding="utf-8")
        for route in (
    ...
    def test_portal_privilege_requires_explicit_scope(self):
        source = (self.portal_root / "models" / "portal_privilege.py").read_text(encoding="utf-8")
